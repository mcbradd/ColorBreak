...
Displayed amounts also open contextual information directly, with exact unrounded currency, assumptions and available evidence. Product names explain the selected product, quantity and market reference. Charts expose their basis. Values inside an existing control use that control's interaction rather than nesting buttons. Information layers retain the invoking panel, query, sort, expanded group, focus and both scroll axes; only the top layer handles dismissal. Seller color names open the same member list used by buyers, with full-page card details available from its thumbnails.
...
6. Buyer break format offers Standard (8 Slots) and Custom. In a standard break the checked slots are the slots the buyer already owns and the crossed slots are the ones another buyer took; both leave the pool the next auction draws from, and the decision prices that remaining pool. Custom accepts 1–500 entries, defaults to 120, reserves each of the 17 catch-all category slots, and fills the remaining entries with the highest-market-value card identities. Character cards sharing a character name occupy one named spot.
7. Custom shows sealed market value per spot, pull EV per spot under the shared bulk filter, top named cards switchable between market-price and expected-value ranking, and residual creature-color/card-type EV with named cards excluded.
8. Buyer setup presents break format first, then break contents, then the slot rail, with an assumptions button opposite the Check a bid header and a bulk-filter switch in that same row. The button opens shipping, tax and threshold controls in an information layer that restores its exact opener when dismissed. Every slot carries its own MIN / expected / MAX candle on one shared scale, at the point of decision rather than in a separate panel. Step explanations live behind a help icon, not in standing paragraphs. The latest decision evidence and visualizations remain visible during recalculation and require no disclosure to open.
...
Product entry always exposes a direct Close beside any Back control. Done, Close, Escape and accepted imports never wait for remote valuation. The entry frame follows the visible viewport without document scrolling; only bounded results/content scroll. Set name/code buttons filter locally with each typed character, including the first.

...
## Section evidence and mobile layout (build 15)

The per-value annotation contract above is superseded: `AnswerGroup` collects child evidence into one primary header note, without adding DOM wrappers. Buyer totals, slot charts, value details, bid decisions and large breaks, plus seller summary/planning sections each own a scope. Nested card inspectors retain their own specific evidence. Shared explanations deduplicate sentences, prioritize named omissions and stay within 65 words. `PanelHeading` groups help and estimate controls with separate 44px targets. This fixes the structural cause of eight identical slot notes and repeated table-cell icons; it does not hide duplicate controls using CSS.

`useShareFeedback` replaces both workspaces' repeated expandable share banners with one clipboard toast. Both update the address bar from the existing public share encoder; private costs and receipts remain excluded. The obsolete buyer import Undo and empty composition panel were removed.

The inspector's image inherited a thumbnail class with a fixed 47px height. Its dedicated full-card image class now preserves the portrait aspect ratio. The panel presents the selected finish price once and one integer-denominator physical pull chance. It no longer repeats foil/nonfoil prices or expected-copy math. Contributor grids reserve separate numeric columns and compact large contributions; full exact values remain in accessible row labels.

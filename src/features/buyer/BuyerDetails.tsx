...
<div className="risk-bar" aria-label={`${Math.round(slot.chaseShare * 100)}% of this color's average value comes from its biggest card`}>
      ...
      <details open className="contributors">
        ...
        <CardMemberList key={slot.id} rows={slot.contributors} onInspect={onInspect} groupName={slot.name} emptyMessage={`No cards in ${slot.name.toLowerCase()} are above the current bulk limit.`} />
      </details>
    </section></AnswerGroup>
  );
}
...
export function LargeBreakView({
  ...
  costs: BuyerCosts;
  onAdjustCosts?: (opener: HTMLElement) => void;
}) {
    ...
    <AnswerGroup><section className="large-break-results" aria-label="Large break spot value">
      ...
      <section className={`large-break-decision ${coverageReady ? "is-ready" : "is-blocked"}`} aria-label="One-spot price check">
        ...
        <div className="large-break-cost-fields">
          ...
          <button type="button" className="quiet" aria-haspopup="dialog" onClick={event => onAdjustCosts?.(event.currentTarget)}>Adjust shipping &amp; tax</button>

...
function GlanceResult({ analysis, current, busy, lines }: { analysis: BreakAnalysis; current: boolean; busy: boolean; lines: BreakLine[] }) {
  ...
  const configuredSurpriseSet = analysis.valuation.priceSource === "Seller-entered Surprise Set";
  const configuredCardCount = analysis.valuation.expectedCards;
  ...
  return <AnswerProvider value={answerFactors(analysis.valuation, analysis.outcomeModel.complete, busy, analysis.outcomeOmissions)}><AnswerGroup>
    ...
    <div className="glance-total"><div><span>{configuredSurpriseSet ? "Surprise Set · configured card value" : "Whole break · expected card value"}</span><strong><AnswerValue value={analysis.valuation.sellableEV} detail={configuredSurpriseSet ? "Sum of the seller-entered values for configured Surprise Set cards. Names and amounts have not been independently verified." : undefined} /></strong></div><small>{configuredSurpriseSet ? `${configuredCardCount} seller-entered ${configuredCardCount === 1 ? "card" : "cards"} · values are estimates` : analysis.valuation.threshold > 0 ? `Cards under ${fmt(analysis.valuation.threshold)} excluded` : "All priced cards included"}</small></div>
    <div className="glance-decision" aria-label="Selected spot value">
      ...
      <AnswerGraphic detail={configuredSurpriseSet ? "This is the exact total of card values entered for the selected color team. It is a configured Surprise Set value, not an opening forecast." : simulation.result?.sampleCount === 0 ? "MIN and MAX use the available pack rules. Typical is still being refined. Missing prices or pack details can change the limits." : "Minimum and maximum possible values for the selected spot under the current pack rules and prices, including rare outcomes. Missing data can change the limits."}><div className="glance-range" aria-label={configuredSurpriseSet ? "Configured team value" : "Modeled opening range"}>
      ...
      </AnswerGraphic><p className="glance-average">{configuredSurpriseSet ? "Average across 8 configured teams" : "Average"} <AnswerValue value={mean} />{configuredSurpriseSet ? " · Based on seller-entered card values." : " · MIN–MAX includes rare modeled outcomes."}</p>
    ...
    <div className="glance-colors-heading"><h3>{configuredSurpriseSet ? "Configured color teams" : "Compare colors"}</h3><span>{configuredSurpriseSet ? "Tap a team · configured value" : "Tap a color · average / range"}</span></div>
    <div className="glance-colors" role="group" aria-label="Compare color values">
      {SLOT_IDS.map((id) => {
        const range = simulation.result?.slotDistributions[id];
        const team = analysis.valuation.slots.find((row) => row.id === id);
        const value = team?.sellableEV;
        const teamCardCount = team?.contributors.length ?? 0;
        const { low, high } = probableRange(range, 0, "80");
        return <InformationButton className="glance-color" key={id} title={`${SLOT_NAMES[id]} team`} label={`Inspect ${SLOT_NAMES[id]} value`} preview={configuredSurpriseSet ? `${SLOT_NAMES[id]} team has ${teamCardCount} configured cards, valued at ${fmt(value)} total.` : `${SLOT_NAMES[id]} average ${fmt(value)}. MIN ${fmt(range?.min)} · MAX ${fmt(range?.max)}. Select to see the cards in this team.`} pressed={slot === id} onOpen={() => setSlot(id)} content={<>
          <p>{configuredSurpriseSet ? `${teamCardCount} configured cards · total ` : "Average "}<AnswerValue label={`${SLOT_NAMES[id]} ${configuredSurpriseSet ? "configured" : "average"} value`} value={value} detail={configuredSurpriseSet ? "Uses the values entered for cards assigned to this team." : "Expected card value for this color across possible openings; actual pulls vary."} />{configuredSurpriseSet ? "" : ` · MIN ${fmtCompact(range?.min ?? 0)} · MAX ${fmtCompact(range?.max ?? 0)}`}</p>
          <CardMemberList rows={team?.contributors ?? []} onInspect={setInspected} groupName={`${SLOT_NAMES[id]} team`} />
        </>}>
          ...
          <small>{configuredSurpriseSet ? `${teamCardCount} configured ${teamCardCount === 1 ? "card" : "cards"} · ${fmt(value ?? 0)} total` : `MIN ${fmtCompact(range?.min ?? 0)} · MAX ${fmtCompact(range?.max ?? 0)}`}<AnswerNote detail={configuredSurpriseSet ? "Counts the named cards in this seller-configured team. Its total uses seller-entered card values." : `The bar shows the middle 80% of modeled openings, excluding the most extreme 10% at each end. The marker shows the median. MIN and MAX are numerical limits only; they do not set the bar scale.${simulation.result?.sampleCount === 0 ? " This is a provisional range while sampling finishes." : ""}`} /></small>
        </InformationButton>;
      })}
    </div>

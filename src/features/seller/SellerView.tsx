...
function RangeCandle({ summary, max, bonus = false }: { summary: DistributionSummary; max: number; bonus?: boolean }) {
  const pct = (value: number) => `${chartPosition(value, max)}%`;
  const { low, high, bodyLow, bodyHigh } = probableRange(summary);
  return (
    <span className={`range-candle ${bonus ? "bonus" : "base"}`} aria-label={`${bonus ? "With bonus" : "Current"}: ${fmt(low)} probable low, ${fmt(summary.median)} typical, ${fmt(high)} probable high; MIN ${fmt(summary.min)}, MAX ${fmt(summary.max)}`}>
      <AnswerNote label="What affects this range chart" detail={`${CANDLE_EXPLANATION}${summary.preview ? " This is a provisional range while sampling finishes." : ""}`} />
      <i className="candle-wick" style={{ bottom: pct(low), height: `${chartPosition(high, max) - chartPosition(low, max)}%` }} />
      <i className="candle-body" style={{ bottom: pct(bodyLow), height: pct(Math.max(.01, bodyHigh - bodyLow)) }} />
...
export function UpsideCandles({ base, bonus, bonusLabel, selectedSlot, selectSlot, useRandom, buyerLanded }: { base: BreakAnalysis; bonus: BreakAnalysis; bonusLabel: string; selectedSlot: SlotId; selectSlot: (slot: SlotId) => void; useRandom: boolean; buyerLanded: number }) {
  ...
  if ((baseSimulation.error || bonusSimulation.error) && (!baseSimulation.result || !bonusSimulation.result)) return (
  ...
  const scenarioMaximum = Math.max(1, probableRange(selectedBefore).high, probableRange(selectedAfter).high);
  const colorMaximum = Math.max(1, ...rows.flatMap((row) => [probableRange(row.before).high, probableRange(row.after).high]));
  ...
  return (
    <div className="upside-chart">
      ...
      <div className="scenario-candles" aria-label="Buyer value range by break configuration">
        <div className="scenario-candle-column"><div><RangeCandle summary={selectedBefore} max={scenarioMaximum} /></div><b>Current break</b><small>MIN <AnswerValue value={selectedBefore.min} /> · MAX <AnswerValue value={selectedBefore.max} /></small></div>
        <div className="scenario-candle-column"><div><RangeCandle summary={selectedAfter} max={scenarioMaximum} bonus /></div><b>+ {bonusLabel}</b><small>MIN <AnswerValue value={selectedAfter.min} /> · MAX <AnswerValue value={selectedAfter.max} /></small></div>
      </div>
...
export function SellerScenarioLab({
  ...
  useEffect(() => {
    ...
    setBonusAnalysis(bestAvailableAnalysis([...lines, bonusLine], baseAnalysis.valuation.threshold));
    let cancelled = false;
    Promise.all([
    ...
    }).catch(() => { /* The provisional answer remains available. */ }).finally(() => { if (!cancelled) setBonusPriceLoading(false); });
    return () => { cancelled = true; };
  ...
  return (
    <AnswerGroup><section className="panel seller-scenario-lab">
      ...
      <div className="scenario-controls">
        ...
        <div className="bonus-market-reference"><span>Current pack market price</span><b><AnswerValue value={bonusMarket} detail={bonusPriceLoading ? "Latest known pack price while a newer price loads." : "Sealed pack market reference, which can differ from what you pay."} /></b><small>Daily sealed-product snapshot</small></div>
      ...
      {selected && bonusAnalysis && (
        <>
          <div className="scenario-economics">
            <div><span>Cost used in profit math</span><b><AnswerValue value={bonusCost} detail="Uses your entered cost, then pack market price. Unknown cost counts as $0, which may overstate profit." /></b><small>{bonusCostOverride == null ? "Current market price" : `Your cost · market ${fmt(bonusMarket)}`}</small></div>
            <div><span>Buyer card value added</span><b>+<AnswerValue value={bonusBuyerValue} /></b></div>
            <div><span>New break-even sales</span><b><AnswerValue value={afterBreakEven} detail="Uses the pack cost estimate and your fee and shipping assumptions." /></b></div>
            <div><span>Buyer landed cost at threshold</span><b><AnswerValue value={averageBuyerLanded} /></b></div>
            <div><span>Profit at <AnswerValue value={thresholdSales} /> before</span><b><AnswerValue value={profitAt(thresholdSales, acquisition)} /></b></div>
            <div><span>Profit at <AnswerValue value={thresholdSales} /> with pack</span><b><AnswerValue value={profitAt(thresholdSales, afterCost)} detail="Uses assumed threshold sales and the best available costs. Unknown costs count as $0 and can overstate profit." /></b></div>
            <div><span>Margin with pack</span><b>{thresholdSales ? `${Math.round(profitAt(thresholdSales, afterCost) / thresholdSales * 100)}%` : "No sales"}<AnswerNote detail="Profit divided by assumed sales. A percentage margin does not exist with $0 sales; the dollar result above still applies." /></b></div>
          </div>
...
function SellerEnticement({
  ...
  useEffect(() => {
    ...
    setBonusAnalysis(bestAvailableAnalysis([...lines, bonusLine], 0));
    Promise.all([
  ...
  const packCost = costOverride ?? marketPrice ?? 0;
  ...
  return (
    <AnswerGroup><section className="seller-enticement" aria-label="Enticement"><div className="section-estimate"><AnswerNote primary detail="Bonus pack costs use your entry or the market estimate. Unknown costs can overstate profit." /></div>
      ...
      <div className="enticement-metrics">
        <div><span>Market / pack</span><b><AnswerValue value={marketPrice} detail="Latest loaded pack price. It improves when the lookup finishes." /></b></div>
        <div><span>Cost / qualifying bid</span><b><AnswerValue value={packCost} detail="Uses your cost or market price. An unpriced or unselected booster currently adds $0 cost; choose a booster or enter its cost to improve the estimate." /></b></div>
        <div><span>Pull EV added / pack</span><b><AnswerValue value={evAdded} /></b></div>
        <div><span>If all {transactionCount} clear <AnswerValue value={threshold} /></span><b><AnswerValue value={allCost} detail="Uses entered pack cost, then its market estimate. Unknown cost counts as $0 and may overstate profit." /></b><small>{allEv == null ? "" : `+${fmt(allEv)} modeled pull EV · ${sellerOutcomeLabel(baseProfitAtAll - (allCost ?? 0))}`}</small></div>
      </div>
...
export function SellerView({
  ...
  const estimatedCosts = lines.some((line) => line.myCost == null);
  ...
  const breakEvenTotal = requiredHammer(transactionCount, shipmentCost, acquisition + otherCosts, 0, buyerShipping, marketplace);
  const breakEvenBid = breakEvenTotal / Math.max(1, transactionCount);
  const plannedBid = plannedBidOverride ?? (Number.isFinite(breakEvenBid) ? Math.ceil(breakEvenBid * 100) / 100 : 0);
  ...
  const readiness = actualAcquisitionCents == null
    ? "Estimated economics — update costs when known."
    ...
    <section className="seller-command-center">
      ...
      <AnswerGroup><section className="seller-contents" aria-labelledby="seller-contents-heading"><div className="section-estimate"><AnswerNote primary detail="Product costs use your entries, then market estimates. An unknown product cost currently adds $0." /></div>
        ...
        <div className="seller-break-reconciliation" aria-label="Seller break composition summary">
          ...
          <span id="seller-cost-status" tabIndex={-1} role="status">{estimatedCosts ? "Estimated cost basis ready for rehearsal" : "Cost basis ready"}</span>
        </div>
      ...
      {missingCostLine && <CompactWarning title={<a href={`#seller-cost-${missingCostLine.id}`} onClick={() => focusManualCost(missingCostLine.id)}>Enter your cost for {missingCostLine.productLabel}</a>} summary="Improve the estimated break-even and profit." className="missing-input-warning">
        <p>This product currently adds $0 to acquisition costs. Break-even may be too low and profit too high until you enter its cost.</p>
      </CompactWarning>}
        ...
        {analysis.valuation.slots.map((slot) => {
          ...
          return <div className={`ask-entry ${!eligible ? "ineligible" : ""}`} key={slot.id}>
            <div className={`ask ${unsold ? "unsold" : ""}`}>
              ...
              <b>{unsold ? "Unsold" : <AnswerValue value={asks[slot.id]} detail="Suggested recovery target based on each color’s share of known value. Missing card values can change the split; this is not a recorded sale." />}</b>
      ...
      <AnswerGroup><section className="panel seller-reconciliation" aria-label="Seller actual reconciliation"><div className="section-estimate"><AnswerNote primary detail="Uses recorded receipts only. Missing receipts or costs leave an estimate rather than final profit." /></div>
        ...
        {activeDraft.actualLedger.orders.length > 0 && <ul className="actual-ledger-list">{activeDraft.actualLedger.orders.map((order) => <li key={order.id}><span>{order.slotIds.join(", ")} · <AnswerValue value={order.receiptCents / 100} /> receipt · <AnswerValue value={order.feeCents / 100} /> fee {order.reference ? `· ${order.reference}` : "· receipt reference missing"}</span><button type="button" className="quiet" onClick={() => setPendingLedgerRemoval({ kind: "order", record: order })}>Remove / correct</button></li>)}</ul>}
        ...
        {activeDraft.actualLedger.shipments.length > 0 && <ul className="actual-ledger-list" aria-label="Recorded shipments">{activeDraft.actualLedger.shipments.map((shipment) => {
          ...
          return <li key={shipment.id}><span>{order?.reference || order?.id || "Recorded order"} · <AnswerValue value={shipment.postageCents / 100} /> postage · <AnswerValue value={shipment.packingCents / 100} /> packing · <AnswerValue value={(shipment.postageCents + shipment.packingCents) / 100} /> fulfillment</span><button type="button" className="quiet" onClick={() => setPendingLedgerRemoval({ kind: "shipment", record: shipment })}>Remove / correct</button></li>;
        })}</ul>}

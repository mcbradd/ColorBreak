...
function ManualBudgetCap({ onBack, target, setTarget, shipping, setShipping, hammer, setHammer }: { onBack: () => void; target: number | undefined; setTarget: (value: number | undefined) => void; shipping: number | undefined; setShipping: (value: number | undefined) => void; hammer: number | undefined; setHammer: (value: number | undefined) => void }) {
  ...
  return <section className="bid-live-decision manual-budget-cap" aria-label="Manual budget cap">
    ...
    {manual?.landedCost != null && <div className={`bid-recommendation bid-recommendation-${manual.status === "under" ? "positive" : manual.status === "at" ? "warning" : "negative"}`} aria-live="polite" aria-label="Bid recommendation"><div><small>Bid recommendation</small><strong>{recommendation}</strong></div><p>{manual.status === "under" ? <>Current bid is <b><AnswerValue value={manual.roomUnderCap} /></b> under your Estimated Max Bid of <b><AnswerValue value={manual.maximumHammer} /></b>. Bid only up to <AnswerValue value={manual.maximumHammer} />.</> : manual.status === "at" ? <>Current bid matches your Estimated Max Bid of <b><AnswerValue value={manual.maximumHammer} /></b>. Do not bid higher.</> : <>Current bid is <b><AnswerValue value={manual.hammerAboveMaximum} /></b> over your Estimated Max Bid of <b><AnswerValue value={manual.maximumHammer} /></b>. Stop bidding.</>}</p></div>}

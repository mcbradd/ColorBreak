...
describe("Break Balance values", () => {
  it("uses simulated one-percent bounds and the interquartile range for the candle body", () => {
    ...
    expect(container.querySelectorAll(".balance-ev-marker")).toHaveLength(8);
    expect(container.querySelectorAll(".balance-body")).toHaveLength(8);
    expect(container.querySelector(".slot-W .balance-best")?.textContent).toBe("HIGH$19.00");
    expect(container.querySelector(".slot-W .balance-worst")?.textContent).toBe("LOW$1.00");
    expect(container.querySelector(".slot-W .balance-ev")?.textContent).toBe("EV$10.00");
    ...
    expect(container.querySelector(".slot-W .balance-ev-marker")?.getAttribute("style")).toContain(`left: ${onScale(10)}%`);

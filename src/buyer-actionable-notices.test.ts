...
describe("ranked card column help", () => {
  ...
  it("explains each column as its own paragraph, with the term set apart", () => {
    render(createElement(CardMemberList, { rows: slot.contributors, onInspect: () => {} }));
  ...
  it("gives every ranked row a chance cell and an adds cell", () => {
    ...
    expect(row.querySelector(".card-member-chance")).not.toBeEmptyDOMElement();
    expect(row.querySelector(".card-member-adds")).not.toBeEmptyDOMElement();
  });
});

...
export function calculateBreak(input: ValuationInput): ValuationResult {
  ...
  for (const draw of input.draws) {
    ...
    if (isCollectorOutlier(card, finish)) {
      ...
      if (!omissions.some((item) => item.dedupeKey === dedupeKey)) omissions.push({
        ...
        dedupeKey,
        message: `${cardDisplayName(card, finish)} has no independently verifiable exact pull rate. The current estimate uses community or inferred odds; its contribution can change substantially as better odds become available.`,
        expectedCards: draw.copies,
        material: true,
      });
    ...
    const existing = byCard.get(contributorKey) ?? {
      ...
      sellablePullProbability: 0, pullRateVerified: !isCollectorOutlier(card, finish),
    };
  ...
  const priceOnlyContributors: Contributor[] = [];
  ...
  return {
    ...
    statusReason: status === "verified"
      ...
      : status === "estimated"
        ...
        : unverifiedPullRate
          ? "Some chase-card odds use community estimates or inference. Their priced values remain included while stronger evidence is sought."
...
export interface DecisionAvailability {
  label: "Decision-ready" | "Estimate · older prices" | "Partial estimate";
  detail: string;
  observedAt?: string;
...
export function decisionAvailability(
  ...
  if (eligibility.status === "stale") return {
    label: "Estimate · older prices",
    detail: "Contents are resolved, but the price snapshot is older than six hours; the bid limit remains an estimate.",
    observedAt: eligibility.observedAt, observedSource: eligibility.observedSource, ageMs: eligibility.ageMs,
  };
  const reason = eligibility.affectedGroups[0]?.label ?? "price or product evidence is unavailable";
  return { label: "Partial estimate", detail: `Best available bid estimate; ${reason}`, observedAt: eligibility.observedAt, observedSource: eligibility.observedSource, ageMs: eligibility.ageMs };
}

/**
 * Classifies whether a valuation can drive a buyer or seller decision.  This is
 * evidence metadata only. Stale or incomplete evidence qualifies estimates;
 * it must not suppress calculations, charts, or decision guidance.
 */
